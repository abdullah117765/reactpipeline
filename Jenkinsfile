                ...
                script {
                    ...
                    // Use SSH credentials to execute commands on the EC2 instance
                        ...
                        if(!params.PARAM_NAME){
                            ...
                            bat "ssh -i \"${env.PRIVATE_KEY_PATH}\" ${env.EC2_INSTANCE_USERNAME}@${env.EC2_INSTANCE_IP} \"echo 'after the login';  sudo docker pull hamazzaii5/${env.JOB_NAME}:latest;  sudo docker run -d -p 8000-9000:${exposePort} hamazzaii5/${env.JOB_NAME}:latest\""
