            ...
            withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'awsCredentials']]) {
                ...
                if (!params.PARAM_NAME) {
                   // bat 'ssh -i "C:/Users/axiom/Downloads/jenkins_depEase.pem" ubuntu@43.204.100.61 "bash -c \'sudo docker pull hamazzaii5/reactpipeline:latest\'"'
                    ...
                    bat 'ssh -i "C:/Users/axiom/Downloads/jenkins_depEase.pem" ubuntu@13.235.67.33 "bash -c \'sudo docker pull hamazzaii5/reactpipeline:latest\'"'
                    bat "ssh -i ${env.PRIVATE_KEY_PATH} ${env.EC2_INSTANCE_USERNAME}@${env.EC2_INSTANCE_IP} \"bash -c 'sudo docker run -d -p 8000-9000:${exposePort} hamazzaii5/reactpipeline:latest'\""
