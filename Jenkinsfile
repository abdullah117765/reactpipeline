pipeline {
    ...
    environment {
        ...
        EC2_INSTANCE_IP = '13.235.67.33'  // don't touch this
                ...
                script {
                    ...
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'awsCredentials']]) {
                        ...
                        if (!params.PARAM_NAME) {
                            ...
                            bat "ssh -i ${env.PRIVATE_KEY_PATH} ${env.EC2_INSTANCE_USERNAME}@${env.EC2_INSTANCE_IP} 'bash -c \"sudo docker run -d -p 8000-9000:${exposePort} hamazzaii5/${jobName}:latest\"'"
                        }
                    }
                }
            }
